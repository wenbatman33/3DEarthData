...
//target 起始點
  ...
  function init() {
    ...
    var _loop = function (i) {
      points.push(new setPoint(data[i].la, data[i].lon, data[i].type, i));
      
    };
    var _drawLoop = function (j) {
    
      var newLine;
      if(j>=2){
        //newLine = drawCurve(points[0].position, points[j].position);
        newLine = drawCurve(points[(j-1)].position, points[j].position);
      }else{
        // console.log(points[(j-1)].position);
        newLine = drawCurve(points[(j)].position, points[j].position);
      }
      
      new TWEEN.Tween(newLine).to({currentPoint: 200}, runTime)
      .delay(j * delayTime)
      .easing(TWEEN.Easing.Cubic.Out)
      .onUpdate(function () {newLine.geometry.setDrawRange(0, newLine.currentPoint);lookAtPoint(points[j].position);})
      .start()
      .onComplete(function() { 
    ...
    dataLoop();
    function lookAtPoint(item){
      target.x=item.x/60;
      //console.log(item.x);

      rotation.x += (target.x - rotation.x) * 0.006;
