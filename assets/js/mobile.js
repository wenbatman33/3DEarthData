...
var Earth = function Earth(el, data) {
  ...
  function init() {
    ...
    var _loop = function (i) {
      //console.log(i);
      points.push(new point(data[i].la, data[i].lon, data[i].type, i));
      var newLine = drawCurve(points[0].position, points[i].position);
      new TWEEN.Tween(newLine).to({currentPoint: 200}, 5000)
      .delay(i * 350 + 1500)
  ...
  function setdeviceor() {
      window.addEventListener('deviceorientation', function(event) {
          ...
          beta = event.beta;
          gamma = event.gamma;
          a.innerHTML ="alpha: " + Math.round(alpha);
          b.innerHTML ="beta: " + Math.round(beta);
          g.innerHTML = "gamma: " + Math.round(gamma);
        }, false);
  }
  setdeviceor();
  // -------------------------------------
  ...
  function onTouchMove(event) {
    ...
    target.y = target.y < -PI_HALF ? -PI_HALF : target.y;
    sendPosition(target.x,target.y,true);
  }
  function onTouchEnd(event) {
    ...
    sendPosition(target.x,target.y,false);
  }
  // -------------------------------------
  //   滑鼠控制項
  // -------------------------------------
...
function sendPosition(xNum,yNum,onTouch){
