...
function loop() {
  if(points_data.length>300){
    var totalNum=300;
  }else{
    var totalNum=points_data.length;
  }
  for (var i = 0; i < totalNum; i++) {
    points.push(new setPoint(points_data[i].la, points_data[i].lon,2, i));
...
var _lines_loop = function (i) {
  var pointColor;
  if(lines_data[i].type=="vip"){
    pointColor=brandsPointColor.vip;
  } else if(lines_data[i].type=="abc"){
    pointColor=brandsPointColor.abc;
  }else if(lines_data[i].type=="jr"){
    pointColor=brandsPointColor.jr;
  }else if(lines_data[i].type=="vjr"){
    pointColor=brandsPointColor.vjr;
  }else if(lines_data[i].type=="ming"){
    pointColor=brandsPointColor.ming;
  }else if(lines_data[i].type==undefined){
    pointColor="#ffffff"
  }

  lines.push(new setPoint_2(lines_data[i].la, lines_data[i].lon,2, i,pointColor));
...
function findPoint(temp) {
  for (var j=0; j<points_data.length; j++) {
    ...
    if(temp == points_data[j].id){
      var consultantPoint=setPoint(points_data[j].la,points_data[j].lon ,2,j);
...
function line_loop() {
  if(lines_data.length>200){
    var totaLinelNum=300;
  }else{
    var totaLinelNum=lines_data.length;
  }
...
function setPoint_2(lat, lng, r, i ,pointColor) {
  var position = latLongToVector3(lat, lng, radius);
  var pointGeometry = new THREE.SphereGeometry(r, 20, 20);
  var pointMaterial = new THREE.MeshBasicMaterial({
    color: pointColor,
