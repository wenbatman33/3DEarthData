...
var today = new Date();
var hoursNum = today.getHours();
var _hoursNum;
var isMobile=detectmob();
console.log(isMobile);

if(hoursNum<10){
  _hoursNum="0"+hoursNum;
}else {
  _hoursNum=hoursNum;
}
...
// var API_line = "___data/consultantLine.json";
var API_point = "___data/consultant_info/"+_hoursNum+".txt";
var API_point_2 = "___data/client_data/"+_hoursNum+".txt";
var API_line = "http://webapi.tutorabc.com/GTRWebService/DataRequest.aspx?dataquery=client_attend_list_s";

console.log(API_point);
console.log(API_line);
...
function setConsultantPoint() {
  ...
  for (var j = 0; j< ConsultantPointsData_Array.length ; j++) {
    points.push(new setPoint(ConsultantPointsData_Array[j].la, ConsultantPointsData_Array[j].lon,2, j ,"#ffffff"));
...
function setClientPoint() {
  ...
  for (var j = 0; j< ClientPointsData_Array.length ; j++) {
    console.log(ClientPointsData_Array[j].type);
    var pointColor;
    ...
    points.push(new setPoint(ClientPointsData_Array[j].la, ClientPointsData_Array[j].lon,2, j ,pointColor));
    point.position.set(points[j].position.x, points[j].position.y, points[j].position.z);
    scene.add(point);
  }
}
